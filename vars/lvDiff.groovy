def call(lvVersion, diffingPicRepo = null) {
    ...
    bat "mkdir ${diffDir}"
    bat "git difftool --no-prompt --extcmd=\"'labview-diff.bat' \$LOCAL \$REMOTE diff_dir ${lvVersion}\" origin/master HEAD"
    // Silencing echo so as to not print out the token.
    bat "@python github_commenter.py --token=${GITHUB_DIFF_TOKEN} --pic-dir=${diffDir} --pull-req=${CHANGE_ID} --info=${JOB_NAME} --pic-repo=${diffingPicRepo}"
}
